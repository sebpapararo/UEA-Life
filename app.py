...
def createAccount():
    ...
    if request.form['username'] != '' and request.form['email'] != '' and request.form['password'] != '' and request.form['verifyPassword'] != '':
            ...
            if re.match("^[A-Za-z0-9_-]*$", username):
                ...
                if query_db('SELECT COUNT(email) FROM users WHERE email = "%s"' % email) and \
                   query_db('SELECT COUNT(email) FROM users WHERE email = "%s"' % email)[0].get('COUNT(email)') == 0:
                    ...
                    if query_db('SELECT COUNT(username) FROM profiles WHERE username = "%s"' % email) and \
                       query_db('SELECT COUNT(username) FROM profiles WHERE username = "%s"' % email)[0].get('COUNT(username)') == 0:
                        ...
                        if request.form['password'] == request.form['verifyPassword']:
                            ...
                            if functions.validatePassword(request.form['password']):
                                ...
                                if functions.verifyCaptcha():
                                    ...
                                    userQuery = 'INSERT INTO users(id, created_on, password, salt, email) VALUES ("%s", "%s", "%s", "%s", "%s")' % (userId, dateAndTime, hashedPassword, userSalt, email)
                                    profileQuery = 'INSERT INTO profiles(id, username, last_active, level, school) VALUES ("%s", "%s", "%s", "%s", "%s")' % (userId, username, dateAndTime, level, school)
